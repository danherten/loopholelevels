...
create index live_sessions_profile_started on public.live_sessions (profile_id, started_at desc);

-- Enforce at most one in-progress (ended_at is null) session per profile.
create unique index live_sessions_one_active on public.live_sessions (profile_id) where ended_at is null;
